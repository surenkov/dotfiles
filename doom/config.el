...
(after! gptel
  ...
  (defun my/list-files-tool (path &optional recursive)
    "List files in PATH. If RECURSIVE is non-nil, list recursively."
  ...
  (dolist (custom-tool-plist
           '((:name "ls"
              ...
              :args ((:name "path" :type string :description "Directory path relative to project root.")
                     (:name "recursive" :type boolean :description "List subdirectories recursively." :optional t)))
             (:name "rg"
              :function my/rg-tool
              :category "filesystem"
